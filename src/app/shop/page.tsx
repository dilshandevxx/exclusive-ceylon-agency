    ...
    <main className="min-h-screen bg-stone-50 dark:bg-stone-950">
      ...
      <section className="py-24 px-6 md:px-12">
        <div className="container mx-auto max-w-7xl">
          <div className="flex justify-between items-center mb-12 border-b border-stone-200 dark:border-stone-800 pb-6">
            <span className="text-stone-500 uppercase tracking-widest text-xs font-medium">Showing {products.length} Items</span>
            <div className="flex gap-4">
              <button className="text-stone-900 dark:text-stone-100 uppercase tracking-widest text-xs font-medium hover:text-accent-gold transition-colors">Sort By</button>
              <button className="text-stone-900 dark:text-stone-100 uppercase tracking-widest text-xs font-medium hover:text-accent-gold transition-colors">Filters</button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16">
            {products.map((product, index) => (
              <div key={product.id} className="group cursor-pointer animate-fade-in-up" style={{ animationDelay: `${0.1 * index}s` }}>
                <div className="relative aspect-[3/4] mb-6 overflow-hidden rounded-sm bg-stone-100 dark:bg-stone-900">
                  <Image
                    src={product.image}
                    alt={product.name}
                    fill
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-6 translate-y-full group-hover:translate-y-0 transition-transform duration-500 flex justify-center">
                    <button className="bg-white/90 backdrop-blur-sm text-stone-900 px-6 py-3 text-xs uppercase tracking-widest font-medium hover:bg-stone-900 hover:text-white transition-colors w-full">
                      Quick Add
                    </button>
                  </div>
                </div>
                <div className="text-center">
                  ...
                  <h3 className="font-serif text-lg text-stone-900 dark:text-stone-50 mb-2 group-hover:text-accent-gold transition-colors">{product.name}</h3>
                  <span className="text-stone-600 dark:text-stone-400 font-light">{product.price}</span>
                </div>
              </div>
            ))}
          </div>
